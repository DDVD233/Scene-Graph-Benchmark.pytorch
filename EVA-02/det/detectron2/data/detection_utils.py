...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    attributes = []
    for obj in annos:
        if "attribute_id" in obj and obj["attribute_id"] is not None:
            attributes.append(obj["attribute_id"] - 1)
        else:
            attributes.append(-1)
    attributes = torch.tensor(attributes, dtype=torch.int64)
    target.gt_attributes = attributes

    # Make sure gt_attributes and gt_classes have the same length
    assert len(target.gt_attributes) == len(target.gt_classes)
